"""ERSEC 29.1.1 roadmap coverage audit.

Deterministic offline mapping from the 29.1.1 roadmap to implementation,
...
FEATURES = [
    ("F01", "Unified API and behavior inventory", ["ersec_assurance_compiler.py"], ["tests/test_assurance_compiler.py"], ["docs/integrated-assurance-loop-29.1.1.md"]),
    ("F02", "First-class authorization policy language", ["ersec_assurance_compiler.py"], ["tests/test_assurance_compiler.py"], ["examples/security-behavior-policy-29.1.1.yaml"]),
    ("F03", "Multi-principal multi-tenant fixture orchestration", ["ersec_assurance_fixtures.py"], ["tests/test_assurance_fixtures.py"], ["docs/stateful-assurance-29.1.1.md"]),
    ("F04", "Stateful business-flow verifier", ["ersec_stateful_assurance.py", "ersec_stateful_links.py"], ["tests/test_stateful_assurance.py", "tests/test_stateful_links.py"], ["docs/stateful-assurance-29.1.1.md", "docs/stateful-producer-links-29.1.1.md"]),
    ("F05", "Hybrid semantic and authoritative oracles", ["ersec_hybrid_oracle.py", "ersec_observer_adapters.py"], ["tests/test_hybrid_oracle.py", "tests/test_observer_provenance.py"], ["docs/hybrid-oracle-29.1.1.md", "docs/authoritative-observers-provenance-29.1.1.md"]),
    ("F06", "Metamorphic security testing", ["ersec_metamorphic.py"], ["tests/test_metamorphic_29.py"], ["examples/metamorphic-security-29.1.1.json"]),
    ("F07", "Mutation adequacy", ["ersec_assurance_mutation.py", "ersec_assurance_benchmark_lab.py"], ["tests/test_assurance_mutation.py", "tests/test_assurance_benchmark_lab.py"], ["docs/assurance-benchmark-lab-29.1.1.md"]),
    ("F08", "Runtime control evidence", ["ersec_hybrid_oracle.py", "ersec_observer_adapters.py"], ["tests/test_hybrid_oracle.py", "tests/test_observer_provenance.py"], ["docs/authoritative-observers-provenance-29.1.1.md"]),
    ("F09", "Counterfactual evidence bundles", ["ersec_counterfactual.py", "ersec_release_evidence.py"], ["tests/test_counterfactual.py", "tests/test_release_evidence.py"], ["docs/counterfactual-assurance-29.1.1.md"]),
    ("F10", "Safe stable developer and CI integration", ["ersec_ci_gate.py", "ersec_remediation_verify.py", "ersec.py"], ["tests/test_ci_gate.py", "tests/test_remediation_verify.py"], [".github/workflows/ci.yml", ".github/workflows/ersec-29.1.1-assurance.yml"]),
]
...
GAPS = [
    {"id": "G01", "area": "Runtime observer breadth", "status": "covered", "detail": "29.1.1 now normalizes OTel/OPA/gateway plus read-only database, audit-log, queue/webhook, object-store, payment-sandbox, and identity-provider evidence; adapters parse supplied evidence only."},
    ...
    {"id": "G03", "area": "Concurrent assurance", "status": "covered", "detail": "29.1.1 now compiles bounded race-sensitive scenarios for an authorized harness; target-side concurrent execution remains intentionally outside the offline trust boundary."},
    ...
    {"id": "G06", "area": "Public benchmark proof", "status": "covered", "detail": "The 29.1.1 public-evaluation harness now enforces reproducibility evidence and refuses to fabricate external results; OWASP Benchmark/WAVSEP/Juice Shop cards remain explicitly not_executed until a controlled external run supplies raw evidence."},
]
...
def _exists(root: Path, rel: str) -> bool:
    if rel.startswith("ersec_") and rel.endswith(".py"):
        return (root / "src" / "ersec" / rel).is_file()
