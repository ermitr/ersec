...
def _version_checks(root: Path) -> list[dict[str, Any]]:
    checks=[]
    pyproject=root/"pyproject.toml"; ersec=root/"src" / "ersec" / "ersec.py"
...
def audit(root: str | Path) -> dict[str, Any]:
    ...
    for name in REQUIRED_MODULES:
        ok,msg=_check(root / "src" / "ersec" / name,f"module:{name}"); checks.append({"id":f"module:{name}","pass":ok,"observed":msg})
