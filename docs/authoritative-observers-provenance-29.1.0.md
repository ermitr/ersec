# ERSEC 29.1.0 — Authoritative Observers and Provenance

ERSEC 29.1.0 can normalize supplied OTel-like spans, OPA decisions, and API-gateway decisions into a common authoritative observation contract. The adapter is parsing-only: it does not contact telemetry systems.
...
```bash
python ersec.py --observer-adapt examples/authoritative-observers-29.1.0.json --observer-trace-verify examples/authoritative-observers-29.1.0.json --observer-expected-revision rev-29
```
...
```bash
python ersec.py --provenance release-evidence-29.1.0.json --provenance-out provenance-29.1.0.json
python ersec.py --verify-provenance provenance-29.1.0.json
```
