# ERSEC 29.1.0 — Integrated Assurance Loop

ERSEC 29.1.0 now exposes a deterministic orchestration layer that connects the major assurance primitives into one evidence bundle:
...
```bash
python3 ersec.py \
  --assurance-loop /tmp/ersec-29.1.0-assurance.json \
  --assurance-loop-policy examples/security-behavior-policy-29.1.0.yaml \
  --assurance-loop-inventory examples/api-behavior-inventory-29.1.0.json \
  --assurance-loop-stateful examples/stateful-business-flow-29.1.0.yaml \
  --assurance-loop-metamorphic examples/metamorphic-security-29.1.0.json \
  --assurance-loop-baseline examples/counterfactual-baseline-29.1.0.json \
  --assurance-loop-mutated examples/counterfactual-mutated-29.1.0.json
```
