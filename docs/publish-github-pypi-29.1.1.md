# ERSEC 29.1.1 — GitHub and PyPI publication

This document is the controlled publication procedure for ERSEC 29.1.1. The release workflow builds distributions once, validates them, stores the exact artifacts, and publishes those artifacts through PyPI Trusted Publishing.
...
From the ERSEC 29.1.1 source directory:

```bash
git init
git branch -M main
git add .
git commit -m "ERSEC 29.1.1"
...
```text
.github/workflows/testpypi-29.1.1.yml
```
...
The public release identifier is exactly `29.1.1` and the Git tag is exactly `v29.1.1`.

```bash
git tag -a v29.1.1 -m "ERSEC 29.1.1"
git push origin v29.1.1
```
...
Create a GitHub Release for tag `v29.1.1`. Use the ERSEC 29.1.1 release notes and attach the generated source/archive artifacts if desired. The workflow publishes the Python distributions from the validated CI artifact rather than rebuilding during publication.
...
python -m pip install --upgrade ersec==29.1.1
ersec --version
ersec --self-test
```

The installed version must report `ERSEC 29.1.1` and the self-test must pass.
