# ERSEC 29.1.1 — Runtime Architecture and Performance Split

ERSEC 29.1.1 keeps the existing public `ersec` import surface while splitting the runtime entry point into three layers:
...
The offline HTML dashboard was upgraded for 29.1.1 with a denser security-operations cockpit layout, improved typography using local/system font stacks, KPI telemetry, responsive layouts, clearer hierarchy, keyboard-friendly controls, reduced-motion support, and no external font/CDN dependency.
...
### Differential Identity Matrix (29.1.1)
