...
from ersec_roadmap_audit import audit

def _tree():
    root = Path(tempfile.mkdtemp())
    for name in ["ersec_assurance_compiler.py", "tests/test_assurance_compiler.py", "examples/security-behavior-policy-29.1.0.yaml"]:
...
def test_real_tree_has_all_feature_anchors():
    ...
    assert result["status"] == "PASS"  # all 29.1.0 engineering roadmap anchors are implemented; external execution is an explicit validation gate
    assert result["explicit_remaining_gaps"] == []
