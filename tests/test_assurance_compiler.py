...
from ersec_assurance_compiler import (
...
def test_policy_compile_is_deterministic_and_versioned():
    doc = load_document(str(ROOT / "examples" / "security-behavior-policy-29.1.0.yaml"))
...
def test_inventory_normalizes_and_deduplicates():
    result = build_inventory(load_document(str(ROOT / "examples" / "api-behavior-inventory-29.1.0.json")))
...
def test_runtime_controls_never_promote_configured_to_enforced():
    doc = load_document(str(ROOT / "examples" / "runtime-control-evidence-29.1.0.json"))
...
def test_evidence_merge_preserves_not_tested():
    policy = load_document(str(ROOT / "examples" / "security-behavior-policy-29.1.0.yaml"))
