...
The 29.1.1 built-in suite should complete with `Ran 66 tests` and `OK`.

## 4. Compile the reviewed security behavior policy

```bash
ersec --assurance-policy-compile \
  examples/security-behavior-policy-29.1.1.yaml \
  --assurance-policy-out assurance-plan-29.1.1.json
```
...
ersec --assurance-inventory \
  examples/api-behavior-inventory-29.1.1.json \
  --assurance-inventory-out inventory-29.1.1.json
```
...
ersec --assurance-controls \
  examples/runtime-control-evidence-29.1.1.json \
  --assurance-controls-out controls-29.1.1.json
```
...
ersec \
  ...
  --sarif ersec-29.1.1.sarif \
  --junit ersec-29.1.1.xml
```

Then compile the Security Reality artifact:

```bash
ersec --reality-model scan-29.1.1.json --reality-out reality-29.1.1.json
```

Then Assurance Intelligence:

```bash
ersec --assurance-intelligence reality-29.1.1.json \
  --assurance-intelligence-out intelligence-29.1.1.json \
...
```bash
ersec --assurance-kernel reality-29.1.1.json \
...
```bash
ersec --verify-assurance-proof release-assurance-29.1.1.json
```

## 8. Kali package installation

For a packaged install, install the 29.1.1 Debian package with:

```bash
sudo apt install ./ersec_29.1.1-1_all.deb
```
...
```bash
ersec --assurance-fixture examples/disposable-multiprincipal-fixture-29.1.1.json \
  --assurance-fixture-out fixture-29.1.1.json
```
...
```bash
python3 ersec.py --concurrent-assurance-compile examples/assurance-concurrency-29.1.1.json --concurrent-assurance-out concurrent-assurance-29.1.1.json
```

Evaluate evidence returned by an authorized harness:

```bash
python3 ersec.py --concurrent-assurance-evaluate concurrent-assurance-29.1.1.json evidence.json --concurrent-assurance-out concurrent-result-29.1.1.json
```

This planner does not itself issue concurrent requests.

## 10. 29.1.1 maturity/reproducibility validation
...
Then install the generated `ersec_29.1.1-1_all.deb` and run:
